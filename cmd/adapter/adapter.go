	...
	return func(params oauth.AdapterParams) {
		...
		if errStr == "no rows in result set" {
			dbReturns, err := db.NewOAuthUserTransaction(context.Background(), dbType.NewOAuthUserTransactionParams{
				...
				Image:             pgtype.Text{String: params.UserData.Picture, Valid: true},
				Username:          params.UserData.Name,
				...
				ExpiresAt:         pgtype.Timestamp{Time: time.Unix(int64(params.TokenData.ExpiresIn), 0), Valid: true},
